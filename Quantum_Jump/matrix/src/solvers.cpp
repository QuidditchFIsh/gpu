...
void mcSolve(Qmatrix& H,Qmatrix& inital,std::vector<Qmatrix>& Cps,double dt,double gamma)
{
	...
	for (int j = 0; j < 2000;j++)
	{
		...
		if(j == 0)
		{
			Evolved = inital - ((_ONEI * dt) * H * inital);
			for (int i = 0; i<Cps.size();i++)
			{
...
void mcSolve(std::vector<H_part> H,Qmatrix& inital,std::vector<Qmatrix>& Cps,double dt,double gamma)
{
		std::ofstream file;
	file.open("Data.dat");
	Qmatrix Expt = PauliZ();
	...
	for (int j = 0; j < 2000;j++)
	{
		...
		if(j == 0)
		{
			...
			{
				Evolved = Evolved - ((_ONEI * dt) * H[k].H_timeDep(j*dt) * inital);
			}
			for (int i = 0; i<Cps.size();i++)
			{
				Evolved = Evolved - ((dt*0.5*gamma) * Cps[i]* Cps[i].hermitian_conj() * inital);
		...
		else
		{
			for(int k = 0; k < H.size();k++)
			{
				Evolved = Evolved - ((_ONEI * dt) * H[k].H_timeDep(j*dt) * Evolved);
			}
			for (int i = 0; i<Cps.size();i++)
			{
				Evolved = Evolved - ((dt*0.5*gamma) * Cps[i]* Cps[i].hermitian_conj() * Evolved);
